...
- The Client has two areas. `ImageKitClient::files()` has `upload(UploadRequest)`, `delete(string $fileId)`, `list(ListRequest)` and `lazy(ListRequest)`. `ImageKitClient::urls()` has `build(UrlRequest)`. There are no other methods.
...
- In tests, call `ImageKitClient::fake()` and assert on it. Do not use `Http::fake()` to stub ImageKit; the fake records uploads, deletions and listings and never sends a request.
